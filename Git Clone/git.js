      ...
      .then((bdata) => {
        ...
        fetch(bdata.url)
          ...
          .then((p) => {
            ...
            row.innerHTML += `<div> <img src=${p[1].owner.avatar_url}><button onclick="repo()" class="ms-5" > Search Repo </button> </div>
                <br>`;
...
function repo() {
  ...
  fetch("https://api.github.com/users/" + a + "/repos").then((bdata) => {
    console.log(bdata);
    fetch(bdata.url)
      .then((c) => c.json())
      .then((p) => {
